...
import { Mail, Lock, Eye, EyeOff, Loader2, Shield, ArrowRight, CheckCircle2 } from 'lucide-react'
...
export default function AdminLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      // Step 1 — sign in via browser client (sets cookie in document.cookie automatically)
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--color-background)' }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] p-12 relative overflow-hidden"
        style={{ background: 'linear-gradient(160deg, hsl(224 75% 18%) 0%, hsl(224 75% 10%) 100%)' }}>
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-40 -left-40 w-80 h-80 rounded-full opacity-20"
            style={{ background: 'radial-gradient(circle, hsl(224 75% 60%), transparent 70%)' }} />
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-16">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center"
              style={{ background: 'linear-gradient(135deg, hsl(224 75% 55%), hsl(262 60% 55%))' }}>
          ...
          <p className="text-base mb-10" style={{ color: 'hsl(224 40% 70%)' }}>
            Manage employees, verify profiles, oversee payroll and attendance from one unified dashboard.
          </p>
          {['Profile verification & approval', 'Manager assignment', 'Payroll & leave management', 'Attendance overview'].map(f => (
            <div key={f} className="flex items-center gap-3 mb-3">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: 'hsl(224 75% 60%)' }} />
              <span className="text-sm" style={{ color: 'hsl(224 30% 75%)' }}>{f}</span>
            </div>
          ))}
        </div>
        <p className="relative text-xs" style={{ color: 'hsl(224 20% 50%)' }}>Human Resource Management System</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--color-foreground)' }}>Admin Sign In</h1>
            <p className="text-sm" style={{ color: 'var(--color-muted)' }}>Access the administrator dashboard</p>
          </div>
          ...
          <div className="mt-6 pt-5 flex items-center justify-center gap-4 text-sm"
            ...
            <Link href="/manager/login" style={{ color: 'var(--color-muted)' }}
              className="hover:opacity-80 transition-opacity">Manager</Link>
          </div>
        </div>
      </div>
    </div>
