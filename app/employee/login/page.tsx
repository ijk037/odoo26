...
import { Mail, Lock, Eye, EyeOff, Loader2, UserCircle, ArrowRight, CheckCircle2 } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--color-background)' }}>
      <div className="hidden lg:flex flex-col justify-between w-[45%] p-12 relative overflow-hidden"
        style={{ background: 'linear-gradient(160deg, hsl(142 50% 12%) 0%, hsl(142 40% 8%) 100%)' }}>
        ...
        <div className="relative">
          ...
          <h2 className="text-4xl font-bold text-white leading-tight mb-4">Your Work,<br />Your Day</h2>
          <p className="text-base mb-10" style={{ color: 'hsl(142 30% 65%)' }}>
            Check in, track your tasks, apply for leave, and manage your profile — all in one place.
          </p>
          {['QR-based attendance check-in', 'Daily task tracking', 'Leave management', 'Salary & payslips'].map(f => (
            <div key={f} className="flex items-center gap-3 mb-3">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: 'hsl(142 70% 45%)' }} />
              <span className="text-sm" style={{ color: 'hsl(142 20% 70%)' }}>{f}</span>
            </div>
          ))}
        </div>
        <p className="relative text-xs" style={{ color: 'hsl(142 15% 40%)' }}>Human Resource Management System</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--color-foreground)' }}>Welcome back</h1>
            <p className="text-sm" style={{ color: 'var(--color-muted)' }}>Sign in to your employee account</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, hsl(142 60% 38%), hsl(224 75% 50%))' }}
            >
              {loading
          ...
          <div className="mt-5 pt-5 flex items-center justify-center gap-4 text-sm"
